...
// 6번 다른 예시
// 같은 필드에 조건을 2번 걸 경우 옵티마이저가 마지막 조건만 적용함. and조건으로 묶어야 함
db.movies.find(
    {
        runtime: {$lte: 100},
        genres: 'Drama',
...
// 7. movies collection에서 runtime이 100분 이하이고 type이 시리즈가 아니고 개봉년도가 2015년 이상이거나 1925년 이하인 영화를 찾는다.
db.movies.find(
    {
        $and : [
            {runtime: {$lte: 100}},
...
// 강사님 쿼리랑 같은데 왜 나는 1925년 이하 데이터 안나오지
...
// 11. restaurants collection에서 Queens에 있는 음식점 중에, 
// grades의 score가 하나라도 70 이상인 document를 조회하고 grades 배열에는 70이 넘는 document 하나만 출력한다.
//  (나머지 필드는 그대로 출력한다)
db.restaurants.find(
    {
        borough: 'Queens',
